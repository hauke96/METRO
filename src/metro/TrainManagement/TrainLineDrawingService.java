...
public class TrainLineDrawingService
{
	 ...
	 * 
	 * @param lineList
	 *            An list of all existing train lines.
	 */
	 ...
	 * 
	 * @param array
	 *            The array of train lines.
	 * @param element
	 *            The element which position you want to know.
	 ...
	 * 
	 * @param offset
	 *            The offset of the game screen.
	 * @param position
	 *            The position of one node.
	 * @param positionNext
	 *            The position of another node.
	 */
	private void drawColoredLine(Point offset, Point position, Point positionNext)
	{
		Point diff = new Point((position.y - positionNext.y) / _gameState.getBaseNetSpacing(), (position.x - positionNext.x) / _gameState.getBaseNetSpacing());
		
		if (diff.x == 1 && diff.y == 1) diff.x = -1;
		else if (diff.x == 1 && diff.y == -1) diff.y = 1;
		Draw.Line(position.x, position.y, positionNext.x, positionNext.y, _lineThickness);
	}
}
