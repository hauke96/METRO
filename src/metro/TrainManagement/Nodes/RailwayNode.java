...
public class RailwayNode
{
	 ...
	 * 
	 * @param position
	 *            The position (NOT pixel) of the node. {@code null} indicates that the node will not be added to the overseer (because it's invalid).
	 * @param neighbor
	...
	public void draw(Point offset)
	{
		...
		position = new Point(offset.x + _position.x * gameState.getBaseNetSpacing(), offset.y + _position.y * gameState.getBaseNetSpacing()); // Position with offset etc.
	 ...
	 * @param train
	 *            The train that's allowed to drive in the part between this node and the successor.
	 */
	 ...
	 * 
	 * @param successor
	 *            The successor of the signal to determine the direction of it.
	 * @param train
	 *            The train that wants to pass the signal.
