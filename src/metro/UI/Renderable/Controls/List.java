...
public class List extends ControlElement
{
	 ...
	 *            The position on the window.
	 * @param compact
	 *            Set to true if list should be a compact list (entries are not that high)
	 */
	 ...
	 *            The position on the window.
	 * @param entries
	 *            The entries that are in the list after creating it.
	 */
	...
	private void drawEntries()
	{
		...
		for (int i = 0; i < _entries.size(); ++i)
		{
			...
			{
				Draw.Line(_area.x + 8, _area.y + _offset + (yOffset - ySpace) + 4, _area.x + _area.width - 11, _area.y + _offset + (yOffset - ySpace) + 4);
			}
	...
	private void drawScrollbar()
	{
		...
		Fill.Rect(_area.x + _area.width - 3, _area.y + yPos, _area.x + _area.width - 2, height); // right for scroll bar
	}
